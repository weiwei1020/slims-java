...
@Service
@Transactional
public class ConsumableReturnServiceImpl implements ConsumableReturnService {
    @Autowired
    ...
    public PageReturn addConsumableReturn(ConsumablePurchase consumablePurchase) {
        ...
        if (consumablePurchaseMapper.insertConsumableReturn(consumablePurchase) == true){
    ...
    public PageReturn selectAllConsumableReturn(){
        PageReturn pageReturn=new PageReturn();
        pageReturn.setCode(1);
        pageReturn.setData(consumablePurchaseMapper.selectAllConsumablePurchase(2));
        pageReturn.setMsg("成功");
        pageReturn.setSuccess(true);
        return pageReturn;
    }
    ...
    @Override
    public Pagelims pageConsumableReturn(int currentPage, int pageSize, ConsumablePurchase consumablePurchase) {
    ...
    public PageReturn useUpConsumable(ConsumablePurchase consumablePurchase) {
        ...
        consumablePurchase.setLtime(slimsTools.gainSystemTime());
        consumablePurchase.setPurchaseNumber("HC120"+slimsTools.gainOrderNumber());
        consumablePurchase.setPurchaseType(2);

        if (consumablePurchaseMapper.useUpConsumable(consumablePurchase) == true){
            pageReturn.setData("0");
            pageReturn.setCode(1);
            pageReturn.setMsg("成功");
            pageReturn.setSuccess(true);
